function [u] = manySprings2(k, m, dt, nt, u10, un0, nmasses, force_fn)
...
for ii=2:nt-1
    ...
    u(ii+1,nmasses) = (k/m)*dt^2*(u(ii,nmasses-1)-2*u(ii,nmasses))+2*u(ii,nmasses)-u(ii-1,nmasses)+(1/m)*force_fn(ii+1,nmasses)*dt^2;
end
